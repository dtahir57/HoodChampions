	      ...
	      <div class="row"> 
	        ...
	        <div class="col-lg-5 offset-lg-2 col-sm-6">
	          ...
	          <div class="group-btns">
	            <button class="btn btn-default" :disabled="!agreed" @click="update">Update</button>
...
export default {
	...
	methods: {
        ...
        update () {
        	if (this.$validator.validate()) {
        		let uri = '/api/lost_and_founds/update';
        		axios.post(uri, {
        			id: this.data.id,
        		...
        		.then(response => {
        			...
        			if (response.data.code == 200) {
        				this.$router.push({ name: 'LostAndFoundView', params: {id:response.data.lost_and_found.id} })
        			}
        		}).catch(error => {
	...
	created () {
		...
		axios.get(uri, {
			...
			this.data = response.data.lost_and_found
		}).catch(error => {
			console.log(error.response)
		})
		let uri2 = '/api/lost_and_found/create';
		axios.get(uri2, {
			headers: {
				"Accept": "application/json",
				"Authorization": `Bearer ${localStorage.getItem('user_api_token')}`
